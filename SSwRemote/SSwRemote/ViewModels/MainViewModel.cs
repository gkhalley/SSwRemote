...
namespace SSwRemote
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        {            
            SSwClient = new SSwClient();
            HostList = new ObservableCollection<ServerItem>();

            SendCommand = new Command(async (s) => await SendSSwCommand((string)s));

            ConnectCommand = new Command(async () => await SendSSwCommand("TurnOn"));
        ...
        async Task GetServersAsync ()
        {
            ...
            var servers = await SSwClient.FindMapleServers();
        ...
        async Task SendSSwCommand(string command)
        {
            ...
            {
                case "TurnOn":
                    if(isSuccessful = await SSwClient.TurnOnAsync(SelectedServer))
                        IsOn = true;
                    break;
                case "TurnOff":
